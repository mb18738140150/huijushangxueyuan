...
@implementation CustomTUIImageMessageCell
...
- (void)fillWithData:(TUIImageMessageCellData *)data;
{
    ...
    if ([data.path containsString:@"http"]) {
        
        [self.thumb sd_setImageWithURL:[NSURL URLWithString:data.path] placeholderImage:[UIImage imageNamed:@""] options:SDWebImageAllowInvalidSSLCertificates progress:nil completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            ;
        }];
        ;
        return;
