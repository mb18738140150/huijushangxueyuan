...
#import "CustomTUIMessageController.h"
...
@interface CustomTUIMessageController () <TMessageCellDelegate>
...
@implementation CustomTUIMessageController
...
- (void)setupViews
{
    ...
    [self.tableView registerClass:[CustomTUITextMessageCell class] forCellReuseIdentifier:TTextMessageCell_ReuseId];
    [self.tableView registerClass:[TUIVoiceMessageCell class] forCellReuseIdentifier:TVoiceMessageCell_ReuseId];
    [self.tableView registerClass:[CustomTUIImageMessageCell class] forCellReuseIdentifier:TImageMessageCell_ReuseId];
...
- (void)getMessagesWithInfo:(NSArray *)msgs msgCount:(int)msgCount
{
    ...
    NSMutableArray *uiMsgs = [self transUIMsgFromIMMsgInfo:msgs];
    dispatch_async(dispatch_get_main_queue(), ^{
        if(msgs.count < msgCount){
            self.noMoreMsg = YES;
            self.indicatorView.mm_h = 0;
...
- (NSMutableArray *)transUIMsgFromIMMsg:(NSArray *)msgs
{
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationOfAddChatRecord object:msg.textElem.text];
        data = [self getMessageDataWith1:msg.textElem.text];
        ...
        if(data) {
            ...
            if ([data isKindOfClass:[TUISystemMessageCellData class]]) {
                ...
                TUISystemMessageCellData * systemData = (TUISystemMessageCellData *)data;
                // 禁言消息
                if ([systemData.content containsString:@"禁言"]) {
                    [uiMsgs addObject:data];
                }else
                {
                    // 礼物消息
...
- (NSMutableArray *)transUIMsgFromIMMsgInfo:(NSArray *)msgs
{
    NSMutableArray *uiMsgs = [NSMutableArray array];
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        
        NSDictionary * infoDic = msgs[k];
...
//        }
        ...
        NSDate * time = [formatter dateFromString:[infoDic objectForKey:@"create_time"]];
        TUISystemMessageCellData *dateMsg = [self transSystemMsgFromDate:time];
...
- (void)sendMessage:(TUIMessageCellData *)msg
{
    ...
    TUITextMessageCellData * textData = msg;
    NSString * jsonStr = textData.content;
    
    TUIMessageCellData * messageData = [self getMessageDataWith:jsonStr];
...
//        _msgForDate = imMsg;
    ...
    if ([messageData isKindOfClass:[TUISystemMessageCellData class]]) {
        TUISystemMessageCellData * systemData = (TUISystemMessageCellData *)messageData;
        if ([systemData.content containsString:@"禁言"]) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        @strongify(self)
        if(messageData.status == Msg_Status_Sending){
            [self changeMsg:messageData status:Msg_Status_Sending_2];
        }
    });
}
...
- (TUIMessageCellData *)getMessageDataWith1:(NSString *)jsonStr
{
    NSString * buyStr = jsonStr;
    jsonStr = [jsonStr stringByReplacingOccurrencesOfString:@"&quot;" withString:@"\""];
    if([jsonStr containsString:@"temp_msg"] )
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationOfMockVIPBuyNotification object:buyStr];
        TUIMessageCellData *data;
        return nil;
    }
    ...
    NSDate * time = [formatter dateFromString:[NSString stringWithFormat:@"%@", [infoDic objectForKey:@"create_time"]]];
    TUISystemMessageCellData *dateMsg = [self transSystemMsgFromDate:time];
    ...
    TUIMessageCellData *data  = [self getNewMessageData:infoDic];
    
    return data;
}

- (TUIMessageCellData *)getNewMessageData:(NSDictionary *)infoDic
{
    TUIMessageCellData *data  ;
    ...
    if ([[infoDic objectForKey:@"msgtype"] isEqualToString:@"image"]) {
        imageData.path = [infoDic objectForKey:@"content"];
        imageData.name = [infoDic objectForKey:@"from_client_name"];
    ...
    }else if([[infoDic objectForKey:@"msgtype"] isEqualToString:@"text"])
    {
        textData.content = [infoDic objectForKey:@"content"] ;
        textData.name = [infoDic objectForKey:@"from_client_name"];
    ...
    }else if([[infoDic objectForKey:@"msgtype"] isEqualToString:@"reward"])
    {
        ...
        data = system;
    }else if([[infoDic objectForKey:@"msgtype"] isEqualToString:@"shutup"])
    {
        TUISystemMessageCellData *system = [[TUISystemMessageCellData alloc] initWithDirection:MsgDirectionOutgoing];
        system.reuseId = TSystemMessageCell_ReuseId;
        
        BOOL isOn;
        if ([[infoDic objectForKey:@"content"] isEqualToString:@"on"]) {
            isOn = YES;
            system.content = @"禁言开启";
        }else
        {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationOfShutUpNotification object:@{@"state":@(isOn)}];
        data = system;
    }
    
    if(data) {

        data.msgID = [infoDic objectForKey:@"msg_id"];
        data.direction =  MsgDirectionIncoming;
        data.identifier = [NSString stringWithFormat:@"%@", [infoDic objectForKey:@"uid"]];
        data.name = [infoDic objectForKey:@"from_client_name"];
        data.avatarUrl = [NSURL URLWithString:[infoDic objectForKey:@"headimg"]];
        data.status = Msg_Status_Succ;
        if ([data isKindOfClass:[TUISystemMessageCellData class]]) {
            data.showName = NO;
        }else
...
- (TUIMessageCellData *)getMessageDataWith:(NSString *)jsonStr
{
    NSData * jsondata = [jsonStr dataUsingEncoding:NSUTF8StringEncoding];
    ...
    if ([[infoDic objectForKey:@"type"] isEqualToString:@"temp_msg"]) {
        return nil;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if(scrollView.contentOffset.y <= TMessageController_Header_Height){
        [self loadMessage];
        if (self.getChatRecordBlick) {
            self.getChatRecordBlick(@{});
        }
    }
}
