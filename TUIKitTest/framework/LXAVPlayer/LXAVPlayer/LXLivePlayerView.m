...
@interface LXLivePlayerView()<UIGestureRecognizerDelegate,TXLivePlayListener>
// 播放器的几种状态
typedef NS_ENUM(NSInteger, LXLivePlayerState) {
    LXLivePlayerStateFailed,     // 播放失败
    LXLivePlayerStateBuffering,  // 缓冲中
    LXLivePlayerStatePlaying,    // 播放中
    LXLivePlayerStateStopped,    // 停止播放
    LXLivePlayerStatePause,       // 暂停播放
    LXLivePlayerStateEnd         // 播放完成
};
...
@property(nonatomic,assign)LXLivePlayerState         playState;//播放状态
...
@end
@implementation LXLivePlayerView

- (int)livePlayerType {
    int playType = -1;
    NSString *videoURL = _currentModel.playUrl;
...
-(void)setCurrentModel:(LXPlayModel *)currentModel{
    ...
    self.playState = LXLivePlayerStateBuffering;
    //播放前准备
    [self readyToPlay];
}
...
- (void)startPlay
{
    ...
    [self.contollView hideVideoProgress];
...
-(void)resetPlayer{
    ...
    self.playState = LXLivePlayerStateStopped;
...
-(void)setPlayState:(LXLivePlayerState)playState{
    
    _playState = playState;
    
    if (_playState == LXLivePlayerStatePlaying) {
        [self.contollView showLoadingAnimation:NO];
        
    }else if(_playState == LXLivePlayerStateBuffering){
...
-(void)doubleTap:(UITapGestureRecognizer *)tap{
    ...
    if (self.playState == LXLivePlayerStatePause) {
        self.playState = LXLivePlayerStatePlaying;
    }else{
        self.playState = LXLivePlayerStatePause;
    }
...
//
//     self.contollView.startTime = [LXLivePlayerView durationStringWithTime:(NSInteger) self.sumTime];
